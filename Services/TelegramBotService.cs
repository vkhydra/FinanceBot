...
public sealed class TelegramBotService : BackgroundService
{
    ...
    private async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            resultado = await financeMessageProcessor.ProcessarMensagemAsync(new FinanceMessageRequest(corpoMensagem));
        }
        catch (DbUpdateException ex)
        {
            _logger.LogError(ex, "Falha ao acessar o banco durante o processamento da mensagem do Telegram.");
            resultado = FinanceMessageResult.Falha("Nao consegui acessar o banco agora. Tente novamente em instantes.");
        }
